...
<style>
    .stButton>button {
        background-color: #1E6091; color: white; font-weight: bold;
        border-radius: 8px; border: none; padding: 0.6rem 2rem; font-size: 16px;
    }
...
with st.sidebar:
    ...
    vessel_name  = st.text_input("Nombre del barco",  value="AS SVENJA / V.26022N")
    port_loading = st.text_input("Port of Loading",   value="BALBOA")
    st.divider()
    st.markdown("**Reglas aplicadas:**")
    st.markdown("""
    - `*` → **N** (no marine pollutant)
    - `P` → **P** (marine pollutant)
    - Gross Weight vacío → NW × 1.05
    - Sin placeholders en puntos 1 y 7
    - Battery remark auto (UN3556/3171/3480/3481)
    """)
    st.divider()
    st.caption("Desarrollado por Juan Deo · MSC")
...
# ─── Constantes ───────────────────────────────────────────────────────────────
ISO_MAP = {
    45.1:"40HC", 45.0:"40HC", 42.1:"40GP", 42.0:"40GP",
...
def consolidate_ctrs(rows_df):
    ...
    for _, r in rows_df.iterrows():
        lbl = iso_label(r["CtrISOCode"])
        ctr = str(r["CtrNumber"]).strip()
        if ctr not in seen:
            seen.add(ctr)
            groups.setdefault(lbl, []).append(ctr)
...
def generate_approval(booking_ref, rows_df, vessel, pol):
    ...
    imo = imo_fmt(r.get("IMOClass",""))
    pg  = pg_fmt(r.get("PG",""))
    lines.append(f"14) IMO Class / UN No / IMDG Packaging Group: {imo} / {un} / {pg}")
    inq  = safe_e(r.get("InnerQty",""))
    ind  = safe_e(r.get("InnerPackingDescription",""))
    inns = ""
    try:
...
def build_excel(df, vessel, pol):
    ...
    thin = Side(style="thin",  color="BDD1EE")
    med  = Side(style="medium", color=MID)
    ...
    wo.column_dimensions["A"].width = 26
    wo.column_dimensions["B"].width = 100
    wo.merge_cells("A1:B1"); c = wo["A1"]
    c.value = (f"  DG APPROVAL TEMPLATE  |  {datetime.now().strftime('%d %b %Y %H:%M')}"
               f"  |  {len(bookings)} booking(s)  |  {vessel}")
    c.font  = Font(name="Arial", size=11, bold=True, color=WHITE)
    c.fill  = mfill(DARK)
    c.alignment = Alignment(horizontal="left", vertical="center", indent=1)
    ...
    for bki, booking in enumerate(bookings):
        ...
        hdr = wo[f"A{outR}"]
        hdr.value = (f"  BOOKING #{bki+1}  |  {booking}"
                     f"  |  {len(unique_ctrs)} container(s)"
                     f"  |  {len(unique_uns)} UN class(es)")
        hdr.font      = Font(name="Arial", size=10, bold=True, color=WHITE)
        hdr.fill      = mfill(MID)
        hdr.alignment = Alignment(horizontal="left", vertical="center", indent=1)
        wo.row_dimensions[outR].height = 18
        outR += 1
        ...
        for uni, un_val in enumerate(unique_uns):
            ...
            if len(unique_uns) > 1:
                ...
                uh.font      = Font(name="Arial", size=9, bold=True, color=WHITE)
                uh.fill      = mfill(ACC)
                uh.alignment = Alignment(horizontal="left", vertical="center", indent=2)
                wo.row_dimensions[outR].height = 16
                outR += 1

            text = generate_approval(
                booking,
                un_rows if len(unique_uns) > 1 else rows_df,
                vessel, pol
            )
            ...
            tc.fill      = mfill(WHITE)
            tc.alignment = Alignment(vertical="top", wrap_text=True, indent=1)
    ...
    ft.font      = Font(name="Arial", size=9, italic=True, color=WHITE)
    ft.fill      = mfill(DARK)
...
def load_dcr(uploaded_file):
    """
    Lee el DCR correctamente tanto en .xls como .xlsx.
    El archivo DMAXS tiene sheet 'BR' con headers en fila 3 (header=2 en pandas).
    Usa engine 'calamine' que soporta ambos formatos (xlrd 2.0+ ya no lee .xls).
    """
    try:
        ...
        if "BookingRef" not in df.columns:
            return None, "No se encontró la columna 'BookingRef'. Verifica que los headers estén en la fila 3."

        df = df.dropna(subset=["BookingRef"])
        if df.empty:
            return None, "El archivo no tiene datos en la sheet 'BR'."
...
with col1:
    ...
    uploaded = st.file_uploader(
        "Arrastra aquí tu archivo DCR",
        type=["xls", "xlsx"],
        help="Exportación DMAXS — sheet 'BR', headers en fila 3"
    )
...
if uploaded:
    ...
    mp_p_count = sum(
        ...
        if mp_fmt(df[df["BookingRef"]==bk].iloc[0].get("MP","")) == "P"
    )
    bat_count = sum(
        1 for bk in bookings
        if df[df["BookingRef"]==bk].iloc[0].get("UNNumber","").upper() in BATTERY_UNS
    )
    ...
    m4.metric("Con baterías", bat_count)

    with st.expander("Ver detalle de bookings", expanded=False):
        rows = []
        for bk in bookings:
            r   = df[df["BookingRef"]==bk]
            r0  = r.iloc[0]
            rows.append({
                "Booking":  bk,
                "UN(s)":    ", ".join(dict.fromkeys(r["UNNumber"].tolist())),
                "CTRs":     ", ".join(dict.fromkeys(r["CtrNumber"].tolist()))[:60],
                "MP":       mp_fmt(r0.get("MP","")),
    ...
    if st.button("⚡ GENERATE APPROVALS", use_container_width=True):
        ...
        dcr_name = uploaded.name.rsplit(".", 1)[0]
        st.download_button(
            label     = "📥 Descargar Excel con plantillas",
            data      = output_buf,
